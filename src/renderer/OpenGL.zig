...
pub const ExportedFrame = Dmabuf;
...
/// Triple-buffering is required when presenting the render
/// target as a DMABUF since the export process clears the target.
/// With triple-buffering we can ensure that there's always a
/// frame ready to be exported and composited.
...
pub fn init(alloc: Allocator, opts: rendererpkg.Options) !OpenGL {
    ...
    //
    // Not surface type is needed since we assume `EGL_KHR_surfaceless_context`
    // is always available, as we want to render headlessly, not to a native
    // window surface. Despite being an optional extension it is implemented
    // practically universally by all graphics drivers that also support our
    // minimum OpenGL version of 4.3, so we shouldn't even need to query for
    // support here.
    ...
    // Create our context.
    const context: *egl.Context = try .create(display, config, null, &.{
    ...
    errdefer context.destroy(display) catch |err| {
        log.err("failed to destroy context err={}", .{err});
    };

    try display.makeCurrent(null, null, context);
